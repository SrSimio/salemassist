class SalemGame {
    ...
    updateStartButton() {
        ...
        const canStart = this.players.length >= 4;
        this.startGameBtn.disabled = !canStart;
        
        if (canStart) {
            this.startGameBtn.textContent = `🌙 Comenzar los Juicios (${this.players.length} aldeanos)`;
        } else {
            this.startGameBtn.textContent = `🌙 Mínimo 4 aldeanos necesarios (${this.players.length}/4)`;
        }
    }
    ...
    playSound(type) {
        // Simulación de efectos de sonido con console.log
        const sounds = {
            'add': '🔔 Sonido: Aldeano agregado',
            'remove': '💥 Sonido: Aldeano removido',
            'start': '🎺 Sonido: Juego iniciado',
            'phase': '🌙 Sonido: Nueva fase',
            'select': '✨ Sonido: Selección',
            'confirm': '⚡ Sonido: Confirmación',
            'timer': '⏰ Sonido: Timer iniciado',
            'timeup': '🚨 Sonido: Tiempo agotado'
        };
        
        console.log(sounds[type] || '🔊 Sonido desconocido');
    }
    ...
    detectAPKEnvironment() {
        ...
        return isAndroidWebView || isCapacitor;
    }
    
    async initializeVoiceSystem() {
        // Inicializar el nuevo sistema de audio híbrido
        if (typeof HybridAudioManager !== 'undefined') {
            this.audioManager = new HybridAudioManager();
            ...
            console.log('🔊 Sistema de Audio Híbrido inicializado');
            console.log('📊 Info del sistema:', this.audioManager.getSystemInfo());
        } else {
            // Fallback al sistema anterior
            this.detectAPKEnvironment();
            
            if ('speechSynthesis' in window && !this.isAPK) {
                this.speechSynthesis = window.speechSynthesis;
                console.log('🔊 Sistema de voces Web Speech API inicializado (fallback)');
            } else {
                console.log('🔊 Sistema de voces en modo respaldo (APK/HTML5 Audio)');
            }
        }
    ...
    speakWithFallback(text, options) {
        ...
        // Mostrar el texto en consola y como notificación visual
...
function initializeApp() {
    ...
    setTimeout(() => {
        ...
        if (loadingScreen && mainApp) {
            ...
            console.log('✅ Pantalla de carga ocultada, iniciando juego...');
            window.game = new SalemGame();
            console.log('🎮 Juego iniciado correctamente');
        } else {
            console.error('❌ Error: No se encontraron elementos loading-screen o main-app');
        }
    }, 2000);
}

// Para aplicaciones Capacitor, esperar que Capacitor esté listo
if (typeof window.Capacitor !== 'undefined') {
    console.log('📱 Entorno Capacitor detectado, esperando que esté listo...');
